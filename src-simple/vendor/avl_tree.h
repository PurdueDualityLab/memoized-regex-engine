...
#define avl_tree_entry(entry, type, member) \
	(entry == NULL ? NULL : (type*) ((char *)(entry) - offsetof(type, member)))
...
/* Same as avl_tree_lookup(), but returns the largest node <= the requested node.
 * Returns NULL if all nodes are bigger.
 */
static AVL_INLINE struct avl_tree_node *
avl_tree_lookup_pred(const struct avl_tree_node *root,
		const void *cmp_ctx,
		int (*cmp)(const void *, const struct avl_tree_node *))
{
	const struct avl_tree_node *cur = root;
	const struct avl_tree_node *lastLEQ = NULL;

	while (cur) {
		int res = (*cmp)(cmp_ctx, cur);
		if (res < 0) // cmp < cur
			cur = cur->left;
		else if (res > 0) { // cmp > cur
			lastLEQ = cur;
			cur = cur->right;
		}
		else { // Match
	...
	return (struct avl_tree_node*)lastLEQ;
}
...
static AVL_INLINE struct avl_tree_node *
avl_tree_lookup_node_pred(const struct avl_tree_node *root,
		     const struct avl_tree_node *node,
		     int (*cmp)(const struct avl_tree_node *,
				const struct avl_tree_node *))
{
